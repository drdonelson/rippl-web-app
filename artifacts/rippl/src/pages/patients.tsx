...
interface ChunkResult {
  ...
  next_offset: number; total_fetched: number; done: boolean;
}
async function importChunk(officeId: string | null, offset: number): Promise<ChunkResult> {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 25_000);
  try {
    const result = await customFetch<ChunkResult>(`${BASE}/api/import/patients/chunk`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ office_id: officeId, offset, limit: 100 }), signal: controller.signal,
    });
...
async function fetchChunkWithRetry(officeId: string | null, offset: number): Promise<ChunkResult> {
  for (let attempt = 0; attempt < 2; attempt++) {
    try { return await importChunk(officeId, offset); }
    catch (err) { if (attempt === 1) throw err; await new Promise<void>(r => setTimeout(r, 2_000)); }
  }
...
interface OfficeImportRowProps {
  officeKey: string; officeName: string; officeId: string | null;
  active: boolean; phase: ImportPhase; onImport: () => void; onReset: () => void;
}
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          {phase.state === "importing" && (
            <span className="text-xs text-muted-foreground tabular-nums hidden sm:inline">
              {phase.current > 0 ? `Importing… ${phase.current.toLocaleString()} patients added so far` : "Importing…"}
            </span>
          )}
          {phase.state === "done" && (
            <span className="flex items-center gap-1 text-xs text-emerald-400">
              <CheckCircle2 className="w-3.5 h-3.5" /> {phase.imported} new, {phase.skipped} skipped
            </span>
          )}
          {phase.state === "error" && (
            <button onClick={onImport} className="flex items-center gap-1 text-xs text-destructive max-w-[240px] hover:text-destructive/80 transition-colors text-left" title={phase.message}>
              <AlertTriangle className="w-3.5 h-3.5 flex-shrink-0" />
              <span className="truncate">
                {phase.partialImported > 0 ? `Imported ${phase.partialImported.toLocaleString()} patients before error — tap to resume` : phase.message}
              </span>
            </button>
          )}
...
function TierBadge({ tier, totalReferrals }: { tier: string | null | undefined; totalReferrals: number }) {
  ...
  return (
    <span className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[10px] font-semibold border ${config.bg} ${config.color} ${config.border}`}
...
export default function Patients() {
  ...
  const setView = (mode: ViewMode) => {
    setViewMode(mode); try { localStorage.setItem("rippl_patients_view", mode); } catch {}
  };
  ...
  const importTargetOffice = useMemo((): Office | null => {
    ...
    return offices.find(o => o.active && o.name.toLowerCase().includes("brentwood")) ?? offices.find(o => o.active) ?? offices[0] ?? null;
  ...
  const getImportPhase = (key: string): ImportPhase => importPhases.get(key) ?? { state: "idle" };
  ...
  const [sendResult, setSendResult] = useState<null | { sms?: { status: string; reason?: string }; email?: { status: string; reason?: string } }>(null);
  ...
  const [bulkResult, setBulkResult] = useState<null | { total: number; sent: number; failed: number; skipped: number; errors: string[] }>(null);
  ...
  const createReferrer = useCreateReferrer({
    mutation: { onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/referrers"] }); queryClient.invalidateQueries({ queryKey: ["/api/dashboard"] }); setIsAddModalOpen(false); reset(); } }
  });
  ...
  useEffect(() => {
    ...
    QRCode.toCanvas(canvasRef.current, effectiveQrUrl, { width: 256, margin: 2, color: { dark: '#0a1628', light: '#ffffff' } });
  }, [effectiveQrUrl]);

  const copyLink = () => {
    if (effectiveQrUrl) { navigator.clipboard.writeText(effectiveQrUrl); setCopied(true); setTimeout(() => setCopied(false), 2000); }
  };
  ...
  const openSendLinkModal = (referrerId: string) => {
    setSendLinkModalReferrerId(referrerId); setSendChannelSms(true); setSendChannelEmail(false); setSendCustomMessage(""); setSendResult(null);
  };
  const handleSendLink = async () => {
    if (!sendLinkModalReferrerId) return;
    const channels: string[] = [];
    if (sendChannelSms) channels.push("sms"); if (sendChannelEmail) channels.push("email");
    ...
    try {
      const data = await customFetch<{ success?: boolean; sms?: { status: string; reason?: string }; email?: { status: string; reason?: string } }>(
        `/api/referrers/${sendLinkModalReferrerId}/send-link`,
        { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ channels, customMessage: sendCustomMessage.trim() || undefined }) }
      );
      setSendResult(data); toast.success("Referral link sent successfully!"); queryClient.invalidateQueries({ queryKey: ["/api/referrers"] });
    } catch (err) {
      if (err instanceof ApiError) { const serverMsg = (err.data as { error?: string } | null)?.error; toast.error(serverMsg ?? err.message); }
      else toast.error("Network error. Check your connection and try again.");
    } finally { setSendLoading(false); }
  };

  const handleBulkSend = async () => {
    if (!bulkChannelSms && !bulkChannelEmail) { toast.error("Select at least one channel."); return; }
    const channels: string[] = [];
    if (bulkChannelSms) channels.push("sms"); if (bulkChannelEmail) channels.push("email");
    setBulkSending(true);
    try {
      const result = await customFetch<{ total: number; sent: number; failed: number; skipped: number; errors: string[] }>(
        `${BASE}/api/referrers/bulk-send-links`,
        { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ office_id: selectedOfficeId === "all" ? "all" : selectedOfficeId, channels }) }
      );
      setBulkResult(result); queryClient.invalidateQueries({ queryKey: ["/api/referrers"] });
      toast.success(`Sent to ${result.sent} patient${result.sent !== 1 ? "s" : ""}` + (result.failed > 0 ? ` (${result.failed} failed)` : ""));
    } catch (err) { toast.error(err instanceof Error ? err.message : "Bulk send failed"); }
  ...
  const handleImportOffice = useCallback(async (officeKey: string, officeId: string | null) => {
    ...
    let offset = 0, accImported = 0, accSkipped = 0;
    try {
      while (!cancelRef.cancelled) {
        const result = await fetchChunkWithRetry(officeId, offset);
        if (cancelRef.cancelled) break;
        accImported += result.imported; accSkipped += result.skipped; offset = result.next_offset;
        setOfficeImportPhase(officeKey, { state: "importing", current: accImported });
        if (result.done) {
          setOfficeImportPhase(officeKey, { state: "done", imported: accImported, skipped: accSkipped });
          queryClient.invalidateQueries({ queryKey: ["/api/referrers"] }); queryClient.invalidateQueries({ queryKey: ["/api/dashboard"] });
          toast.success(`Import complete — ${accImported.toLocaleString()} patients added`, {
            description: accSkipped > 0 ? `${accSkipped.toLocaleString()} already existed and were skipped.` : "All patients are new enrollees.", duration: 6000,
          }); break;
        }
      }
    } catch (err) {
  ...
  type AnyReferrer = Record<string, unknown>;

  const sendingToday = useMemo(() => {
    const now = new Date();
    const twoHoursLater = new Date(now.getTime() + 2 * 60 * 60 * 1000);
    return (referrers ?? [] as AnyReferrer[]).filter((r: AnyReferrer) => {
      const scheduledRaw = r.onboarding_sms_scheduled_at as string | null;
      const sent = r.onboarding_sms_sent as boolean;
      if (!scheduledRaw || sent) return false;
      const scheduledDate = new Date(scheduledRaw);
      return scheduledDate >= now && scheduledDate <= twoHoursLater;
    }) as AnyReferrer[];
  }, [referrers, todayStart]);

  const sentToday = useMemo(() => {
    return (referrers ?? [] as AnyReferrer[]).filter((r: AnyReferrer) => {
      const sentAtRaw = r.onboarding_sms_sent_at as string | null;
      if (!sentAtRaw) return false;
      const d = new Date(sentAtRaw);
      return d >= todayStart && d <= todayEnd;
    }) as AnyReferrer[];
  }, [referrers, todayStart, todayEnd]);
  ...
  const thSortBtn = (field: SortField, label: string, extraClass = "") => (
    <th className={cn("px-4 py-3 font-semibold text-left", extraClass)}>
      <button onClick={() => handleSort(field)}
        className={cn("flex items-center gap-1.5 text-xs uppercase tracking-wider font-semibold transition-colors whitespace-nowrap",
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card border border-border rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-border flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-foreground text-sm">Sending Today</p>
                <p className="text-xs text-muted-foreground">Onboarding SMS scheduled to fire within the next 2 hours</p>
              </div>
              {sendingToday.length > 0 && (
                <span className="ml-auto px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-400 text-xs font-semibold border border-amber-500/20">
                  {sendingToday.length}
                </span>
              )}
            </div>
            ...
            {isLoading ? (
              <div className="px-5 py-6 space-y-3">
                {[1, 2].map(i => <div key={i} className="h-10 bg-muted/40 rounded-xl animate-pulse" />)}
              </div>
            ) : sendingToday.length === 0 ? (
              <div className="px-5 py-8 text-center text-muted-foreground text-sm">No messages scheduled for today</div>
            ) : (
                  ...
                  return (
                    <div key={r.id as string} className="px-5 py-3 flex items-center gap-4">
                      ...
                      <div className="text-right flex-shrink-0">
                        <p className="text-xs font-medium text-amber-400">Sends at {formatTime(scheduledAt)}</p>
                      </div>
                      <SmsStatusCell
                        onboarded={false}
                        optedOut={optedOut}
          ...
          <div className="bg-card border border-border rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-border flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-foreground text-sm">Sent Today</p>
                <p className="text-xs text-muted-foreground">Onboarding SMS delivered today</p>
              </div>
              {sentToday.length > 0 && (
                <span className="ml-auto px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 text-xs font-semibold border border-emerald-500/20">
                  {sentToday.length}
                </span>
              )}
            </div>
            ...
            {isLoading ? (
              ...
              <div className="px-5 py-8 text-center text-muted-foreground text-sm">No messages sent today yet</div>
            ) : (
              <div className="divide-y divide-border">
                {sentToday.map((r: AnyReferrer) => {
                  const sentAt = r.onboarding_sms_sent_at as string;
                  const officeLabel = officeById.get(r.office_id as string) ?? "—";
                  return (
                    <div key={r.id as string} className="px-5 py-3 flex items-center gap-4">
                      <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground flex-shrink-0">Sent at {formatTime(sentAt)}</p>
                      <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[11px] font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 whitespace-nowrap flex-shrink-0">
                        <Check className="w-2.5 h-2.5" /> Sent
                      </span>
                    </div>
                  );
          ...
          {/* Section 3: This Week (collapsed by default) */}
          <div className="bg-card border border-border rounded-2xl overflow-hidden">
            <button
              onClick={() => setWeekExpanded(v => !v)}
              className="w-full px-5 py-4 flex items-center gap-3 hover:bg-muted/10 transition-colors"
            >
              <div className="w-7 h-7 rounded-lg bg-primary/8 flex items-center justify-center flex-shrink-0">
                <Calendar className="w-3.5 h-3.5 text-primary/70" />
              </div>
              <div className="text-left min-w-0">
                <p className="font-semibold text-foreground text-sm">This Week</p>
                <p className="text-xs text-muted-foreground">Onboarding SMS sent in the last 7 days (not today)</p>
              </div>
            ...
            {weekExpanded && (
              <div className="border-t border-border">
                {isLoading ? (
                  ...
                  <div className="px-5 py-8 text-center text-muted-foreground text-sm">No messages sent this week outside of today</div>
                ) : (
                  <div className="divide-y divide-border">
                    {thisWeek.map((r: AnyReferrer) => {
                      const sentAt = r.onboarding_sms_sent_at as string;
                      const officeLabel = officeById.get(r.office_id as string) ?? "—";
                      return (
                        <div key={r.id as string} className="px-5 py-3 flex items-center gap-4">
                          ...
                          <p className="text-xs text-muted-foreground flex-shrink-0">{formatDate(sentAt)}</p>
                          <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[11px] font-semibold bg-blue-500/10 text-blue-400 border border-blue-500/20 whitespace-nowrap flex-shrink-0">
                            <Check className="w-2.5 h-2.5" /> Sent
                          </span>
                        </div>
                      );
        ...
        <div className="space-y-5">
          ...
          {!isDemo && (
            <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">Pull active patients and enroll them as referrers. Existing patients are skipped automatically.</p>
                  </div>
                </div>
                {importTargetOffice && (
                  <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-primary/10 border border-primary/20 flex-shrink-0 self-start">
                    ...
                    <span className="text-xs text-primary font-medium whitespace-nowrap">
                      {(() => { const d = importTargetOffice.name.lastIndexOf("–"); return d !== -1 ? importTargetOffice.name.slice(d + 2).trim() : importTargetOffice.name; })()}
                      {selectedOfficeId === "all" && <span className="text-primary/60 font-normal"> (default)</span>}
                    </span>
                  </div>
                )}
              </div>
              <div className="divide-y divide-border">
                {!importTargetOffice ? (
                  <OfficeImportRow officeKey="default" officeName="Open Dental" officeId={null} active={true}
                    phase={getImportPhase("default")} onImport={() => handleImportOffice("default", null)} onReset={() => resetOfficeImport("default")} />
                ) : (
                  <OfficeImportRow key={importTargetOffice.id} officeKey={importTargetOffice.id} officeName={importTargetOffice.name}
                    officeId={importTargetOffice.id} active={importTargetOffice.active}
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Search name, email, code…" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                className="pl-9 pr-3 py-2 bg-card border border-border rounded-lg focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary w-full text-sm transition-all" />
            </div>
            <div className="flex items-center gap-2">
              {/* Filter pills */}
              <div className="flex items-center gap-1 p-0.5 bg-card border border-border rounded-xl">
                {([
                  { key: "all", label: "All" },
                  { key: "active", label: "Has Referral" },
                  { key: "opted_out", label: "Opted Out" },
                ] as { key: PatientFilter; label: string }[]).map(({ key, label }) => (
                  <button key={key} onClick={() => setPatientFilter(key)}
                    className={cn("px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-150 whitespace-nowrap",
                      patientFilter === key ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>
                    {label}
                  </button>
                ))}
              </div>
              ...
              <select value={tierFilter} onChange={e => setTierFilter(e.target.value)}
                ...
                <option value="starter">Influencer</option>
                <option value="rippler">Amplifier</option>
              ...
              <div className="flex items-center p-0.5 bg-muted border border-border rounded-lg" style={{ height: "36px" }}>
                <button onClick={() => setView("list")} title="List view"
                  className={cn("flex items-center justify-center w-8 h-full rounded-[5px] transition-all duration-150",
                    viewMode === "list" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground/60 hover:text-muted-foreground")}>
                  <LayoutList className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => setView("grid")} title="Grid view"
                  className={cn("flex items-center justify-center w-8 h-full rounded-[5px] transition-all duration-150",
                    viewMode === "grid" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground/60 hover:text-muted-foreground")}>
          ...
          {isLoading ? (
            viewMode === "grid" ? (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-48 bg-card rounded-2xl border border-border animate-pulse" />)}
              </div>
            ) : (
              <div className="bg-card border border-border rounded-2xl overflow-hidden animate-pulse">
                {Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className="px-5 py-4 border-b border-border flex gap-4">
                    <div className="h-4 w-36 bg-muted rounded" /><div className="h-4 w-20 bg-muted rounded" /><div className="h-4 w-28 bg-muted rounded" />
                  </div>
                ))}
              </div>
            )
              ...
              <div className="block sm:hidden bg-card border border-border rounded-2xl shadow-xl shadow-black/10 overflow-hidden">
                {mobileSortedReferrers.length === 0 ? (
                  ...
                  <div className="divide-y divide-border">
                    {mobileSortedReferrers.map((referrer) => {
                      const code = (referrer as AnyReferrer).referral_code as string | null;
                          ...
                          <div className="flex items-center gap-2.5 min-w-0">
                            <div className="min-w-0">
                              ...
                              <div className="mt-0.5 flex items-center gap-2 flex-wrap">
                                ...
                                {code && <span className="font-mono px-1 py-0.5 rounded bg-background border border-border text-primary" style={{ fontSize: "10px" }}>{code}</span>}
                                <span className="text-xs text-muted-foreground tabular-nums">{n} ref{n !== 1 ? "s" : ""}</span>
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5 flex-shrink-0">
                            <button onClick={() => setQrModalReferrerId(referrer.id)} title="Get QR Code"
                            ...
                            <button onClick={() => openSendLinkModal(referrer.id)} title="Send Referral Link"
                ...
                {!showAllMobile && sortedReferrers.length > MOBILE_ROW_LIMIT && (
                  <div className="py-4 text-center border-t border-border">
                    <button onClick={() => setShowAllMobile(true)} className="text-sm text-primary hover:underline font-medium">
                      ...
                      <tr className="bg-muted/30 border-b border-border">
                        ...
                        <th className="px-3 py-3 text-xs uppercase tracking-wider font-semibold text-muted-foreground whitespace-nowrap">
                          <span className="flex items-center gap-1.5">
                            Auto SMS <span title="Patients receive an automatic referral link SMS 2 hours after their visit. Toggle off to prevent this for specific patients." className="cursor-help opacity-50 hover:opacity-100 transition-opacity"><Info className="w-3 h-3" /></span>
                          </span>
                        </th>
                        <th className="px-3 py-3 text-xs uppercase tracking-wider font-semibold text-muted-foreground text-right">Actions</th>
                      </tr>
                    </thead>
                            ...
                            <td className="px-3 py-2.5">
                              <div className="flex items-center justify-end gap-1">
                                ...
                                <div className="relative">
                                  <button onMouseDown={e => e.stopPropagation()} onClick={() => setActionMenuId(menuOpen ? null : referrer.id)} title="More actions"
                                    className={cn("w-7 h-7 flex items-center justify-center rounded-lg transition-colors border border-border text-muted-foreground",
                ...
                return (
                  <div key={referrer.id} className={cn("bg-card rounded-2xl border border-border p-6 hover:shadow-xl transition-all duration-300 flex flex-col group", gridOptedOut && "opacity-60")}>
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="w-10 h-10 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-lg">{referrer.name.charAt(0)}</div>
                    </div>
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      <div className="bg-background rounded-xl p-3 border border-border">
                        ...
                        <TierProgress tier={(referrer as AnyReferrer).tier as string} totalReferrals={referrer.total_referrals} />
                      </div>
                      <div className="bg-background rounded-xl p-3 border border-border">
                    ...
                    <div className="mt-auto flex gap-2">
                      <button onClick={() => setQrModalReferrerId(referrer.id)}
                      ...
                      <button onClick={() => openSendLinkModal(referrer.id)}
      ...
      {/* Add Patient */}
      <Modal isOpen={isAddModalOpen} onClose={() => { setIsAddModalOpen(false); reset(); }} title="Add New Referrer"
        description="Enroll a patient in the referral program to generate their custom QR code.">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-foreground mb-1.5">Full Name</label>
            <input {...register("name")} className="w-full px-4 py-2.5 bg-background border border-border rounded-xl focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-foreground" placeholder="e.g. Jane Doe" />
            {errors.name && <p className="text-destructive text-xs mt-1">{errors.name.message}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-foreground mb-1.5">Patient Management ID</label>
            <input {...register("patient_id")} className="w-full px-4 py-2.5 bg-background border border-border rounded-xl focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-foreground" placeholder="e.g. PT-12345" />
            {errors.patient_id && <p className="text-destructive text-xs mt-1">{errors.patient_id.message}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-foreground mb-1.5">Phone Number</label>
            <input {...register("phone")} className="w-full px-4 py-2.5 bg-background border border-border rounded-xl focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-foreground" placeholder="(555) 123-4567" />
            {errors.phone && <p className="text-destructive text-xs mt-1">{errors.phone.message}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-foreground mb-1.5">Email (Optional)</label>
            <input {...register("email")} className="w-full px-4 py-2.5 bg-background border border-border rounded-xl focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-foreground" placeholder="jane@example.com" />
          ...
          <div className="pt-4 flex gap-3">
            <button type="button" onClick={() => { setIsAddModalOpen(false); reset(); }}
              className="flex-1 py-3 px-4 bg-muted hover:bg-muted/80 text-foreground rounded-xl font-semibold transition-colors">Cancel</button>
            <button type="submit" disabled={createReferrer.isPending}
      ...
      {/* Individual Patient QR Code Modal */}
      <Modal isOpen={!!qrModalReferrerId} onClose={() => setQrModalReferrerId(null)} title="Patient QR Code">
        <div className="flex flex-col items-center">
          <a href={effectiveQrUrl ?? "#"} target="_blank" rel="noopener noreferrer" title="Open referral link"
          ...
          <div className="w-full space-y-3">
            ...
            <div className="flex gap-2">
              <input readOnly value={effectiveQrUrl || (isDemo ? "Loading demo…" : "Loading...")}
              ...
              <button onClick={copyLink} title="Copy link" className="p-3 bg-secondary hover:bg-muted text-foreground rounded-xl transition-colors">
                {copied ? <Check className="w-5 h-5 text-green-500" /> : <Copy className="w-5 h-5" />}
              </button>
            </div>
            <a href={effectiveQrUrl ?? "#"} target="_blank" rel="noopener noreferrer" onClick={e => { if (!effectiveQrUrl) e.preventDefault(); }}
            ...
            {qrModalReferrerId && (
              <button onClick={() => { setQrModalReferrerId(null); openSendLinkModal(qrModalReferrerId); }}
                ...
                <Send className="w-4 h-4" /> Send Referral Link
              </button>
            )}
          </div>
        </div>
      ...
      <Modal isOpen={!!sendLinkModalReferrerId} onClose={() => setSendLinkModalReferrerId(null)} title="Send Referral Link"
            ...
            <div className="space-y-2">
              {sendResult.sms && (
                <div className={cn("flex items-center gap-3 px-4 py-3 rounded-xl border text-sm font-medium",
                  sendResult.sms.status === "sent" ? "bg-green-500/10 border-green-500/20 text-green-400" : "bg-red-500/10 border-red-500/20 text-red-400")}>
                  <Phone className="w-4 h-4 flex-shrink-0" />
                  <span>SMS: {sendResult.sms.status === "sent" ? "Delivered" : `Failed — ${sendResult.sms.reason ?? "unknown error"}`}</span>
                </div>
              )}
              {sendResult.email && (
                <div className={cn("flex items-center gap-3 px-4 py-3 rounded-xl border text-sm font-medium",
                  sendResult.email.status === "sent" ? "bg-green-500/10 border-green-500/20 text-green-400"
                  : sendResult.email.status === "skipped" ? "bg-muted/50 border-border text-muted-foreground"
                  : "bg-red-500/10 border-red-500/20 text-red-400")}>
                  <Mail className="w-4 h-4 flex-shrink-0" />
                  <span>Email: {sendResult.email.status === "sent" ? "Delivered" : sendResult.email.status === "skipped" ? "No email on file" : `Failed — ${sendResult.email.reason ?? "unknown error"}`}</span>
                </div>
              )}
            </div>
            <div className="flex gap-2 pt-2">
              <button onClick={() => setSendResult(null)} className="flex-1 py-2.5 px-4 bg-muted hover:bg-muted/80 text-foreground rounded-xl font-semibold transition-colors text-sm">Send Again</button>
              <button onClick={() => setSendLinkModalReferrerId(null)} className="flex-1 py-2.5 px-4 bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl font-semibold transition-colors text-sm">Done</button>
            </div>
          </div>
        ) : (
          <div className="space-y-5">
            <div>
              ...
              <div className="flex gap-2">
                <input readOnly value={sendLinkUrl ?? "Loading…"} className="flex-1 px-3 py-2 bg-background border border-border rounded-xl text-xs text-muted-foreground font-mono truncate outline-none" />
                <button onClick={() => sendLinkUrl && navigator.clipboard.writeText(sendLinkUrl)} title="Copy link" className="p-2 bg-secondary hover:bg-muted text-foreground rounded-xl transition-colors border border-border"><Copy className="w-4 h-4" /></button>
              </div>
            </div>
            <div>
              ...
              <div className="flex gap-2">
                {(() => {
                  ...
                  return (
                    <button type="button" disabled={!hasPhone} onClick={() => setSendChannelSms(v => !v)}
                      className={cn("flex-1 flex flex-col items-center gap-1.5 py-3 px-3 rounded-xl border font-medium text-sm transition-all",
                        sendChannelSms && hasPhone ? "bg-primary/10 border-primary/40 text-primary"
                        : !hasPhone ? "opacity-40 cursor-not-allowed bg-muted border-border text-muted-foreground"
                        : "bg-muted/40 border-border text-muted-foreground hover:border-border/80")}>
                      <Phone className="w-4 h-4" /><span>SMS</span>
                      <span className="text-xs font-normal opacity-70 truncate max-w-full px-1">{hasPhone ? phone : "No phone"}</span>
                    </button>
                  );
                })()}
                {(() => {
                  ...
                  return (
                    <button type="button" disabled={!hasEmail} onClick={() => setSendChannelEmail(v => !v)}
                      className={cn("flex-1 flex flex-col items-center gap-1.5 py-3 px-3 rounded-xl border font-medium text-sm transition-all",
                        sendChannelEmail && hasEmail ? "bg-primary/10 border-primary/40 text-primary"
                        : !hasEmail ? "opacity-40 cursor-not-allowed bg-muted border-border text-muted-foreground"
                        : "bg-muted/40 border-border text-muted-foreground hover:border-border/80")}>
                      <Mail className="w-4 h-4" /><span>Email</span>
                      <span className="text-xs font-normal opacity-70 truncate max-w-full px-1">{hasEmail ? email : "No email"}</span>
                    </button>
                  );
                })()}
              </div>
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={() => setSendLinkModalReferrerId(null)} className="flex-1 py-3 px-4 bg-muted hover:bg-muted/80 text-foreground rounded-xl font-semibold transition-colors text-sm">Cancel</button>
              <button onClick={handleSendLink} disabled={sendLoading || (!sendChannelSms && !sendChannelEmail)}
                ...
                {sendLoading ? <><Loader2 className="w-4 h-4 animate-spin" /> Sending…</> : <><Send className="w-4 h-4" /> Send Link</>}
              </button>
            </div>
          </div>
        )}
      ...
      {/* Bulk Send Modal */}
      <Modal isOpen={bulkSendOpen} onClose={() => { if (!bulkSending) setBulkSendOpen(false); }} title="Send All Referral Links"
        ...
        {bulkResult ? (
          <div className="space-y-5">
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className={cn("border rounded-xl p-3 text-center", bulkResult.failed > 0 ? "bg-red-500/10 border-red-500/20" : "bg-muted/40 border-border")}>
                <p className={cn("text-2xl font-display font-bold", bulkResult.failed > 0 ? "text-red-400" : "text-muted-foreground")}>{bulkResult.failed}</p>
            ...
            {bulkResult.errors.length > 0 && (
              <div className="bg-red-500/8 border border-red-500/20 rounded-xl p-3 max-h-32 overflow-y-auto space-y-1">
                ...
                {bulkResult.errors.map((e, i) => <p key={i} className="text-xs text-red-400/80 font-mono leading-snug">{e}</p>)}
              </div>
            )}
            <button onClick={() => setBulkSendOpen(false)} className="w-full py-3 px-4 bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl font-semibold transition-colors text-sm">Done</button>
          </div>
        ) : (
          <div className="space-y-5">
            ...
            <div>
              <p className="text-xs font-medium text-muted-foreground mb-2">Send via</p>
              <div className="flex gap-2">
                <button type="button" onClick={() => setBulkChannelSms(s => !s)}
                  className={cn("flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border font-semibold text-sm transition-all",
                    bulkChannelSms ? "bg-primary/10 border-primary/30 text-primary" : "bg-muted/40 border-border text-muted-foreground hover:border-border/80")}>
                  <Phone className="w-4 h-4" /> SMS
                </button>
                <button type="button" onClick={() => setBulkChannelEmail(s => !s)}
                  className={cn("flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border font-semibold text-sm transition-all",
                    bulkChannelEmail ? "bg-primary/10 border-primary/30 text-primary" : "bg-muted/40 border-border text-muted-foreground hover:border-border/80")}>
            ...
            {bulkSending && (
              <div className="flex items-center justify-center gap-2 py-2 text-sm text-muted-foreground">
                <Loader2 className="w-4 h-4 animate-spin text-primary" /> <span>Sending… this may take a minute</span>
              </div>
            )}
            <div className="flex gap-2 pt-1">
              <button onClick={() => setBulkSendOpen(false)} disabled={bulkSending}
                className="flex-1 py-3 px-4 bg-muted hover:bg-muted/80 text-foreground rounded-xl font-semibold transition-colors text-sm disabled:opacity-40 disabled:cursor-not-allowed">Cancel</button>
              <button onClick={handleBulkSend} disabled={bulkSending || (!bulkChannelSms && !bulkChannelEmail) || unsentCount === 0}
                className="flex-1 py-3 px-4 bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl font-semibold shadow-lg shadow-primary/20 transition-colors text-sm flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                {bulkSending ? <><Loader2 className="w-4 h-4 animate-spin" /> Sending…</> : <><Send className="w-4 h-4" /> Send to {unsentCount} Patient{unsentCount !== 1 ? "s" : ""}</>}
              </button>
            </div>
          </div>
        )}
