...
export default function DemoDental() {
  const { loginAsDentalDemo, session, profile, isLoading } = useAuth();
  ...
  const handleAccess = async () => {
    setError(null);
    setLoading(true);
    const { error: err } = await loginAsDentalDemo();
    setLoading(false);
    if (err) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm text-center space-y-5">
          <div>
            <p className="text-xs font-semibold text-[#E0622A] uppercase tracking-widest mb-2">Brentwood Family Dental</p>
            <h1 className="text-2xl font-bold text-slate-900 leading-tight">
              Your referral dashboard<br />is ready
            </h1>
            <p className="text-sm text-slate-500 mt-2">
              Track patient referrals, manage rewards, and grow your practice — all in one place.
            </p>
          </div>
          ...
          <div className="border-t border-slate-100 pt-4 space-y-2 text-left text-sm text-slate-500">
            <div className="flex items-center gap-2.5">
              ...
              <span>6 active referrers with QR codes</span>
            </div>
            <div className="flex items-center gap-2.5">
              <span className="w-5 h-5 rounded-full bg-orange-100 text-[#E0622A] flex items-center justify-center text-xs font-bold shrink-0">✓</span>
              <span>12 referral events across all stages</span>
            </div>
            <div className="flex items-center gap-2.5">
              <span className="w-5 h-5 rounded-full bg-orange-100 text-[#E0622A] flex items-center justify-center text-xs font-bold shrink-0">✓</span>
              <span>5 Amazon gift card rewards fulfilled</span>
            </div>
          </div>
